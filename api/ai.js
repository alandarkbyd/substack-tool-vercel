...
module.exports = async function (req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({ status: "ok", message: "Groq API is running!" });
  }
  ...
  if (!GROQ_API_KEY) {
    return res.status(500).json({
      error: "GROQ_API_KEY সেট নেই। Vercel → Project Settings → Environment Variables এ যোগ করো।"
    });
  }
  ...
  try {
    ...
    if (result.status !== 200) {
      return res.status(result.status).json({
        error: "[Groq " + result.status + "] " + (result.data?.error?.message || JSON.stringify(result.data))
      });
    }
